...
{
    public class GetPendingReceivedAlliancesQueryHandler : IGetPendingReceivedAlliancesQuery
    {
        private readonly IPoliticalAllienceRepository _allianceRepository;

        public GetPendingReceivedAlliancesQueryHandler(IPoliticalAllienceRepository allianceRepository)
        {
        ...
        public async Task<ValidationResult<IEnumerable<AllianceDto>>> ExecuteAsync(int authenticatedPartyId)
        {
            ...
            try
            {
                var alliances = await _allianceRepository.GetPendingReceivedAsync(authenticatedPartyId);
