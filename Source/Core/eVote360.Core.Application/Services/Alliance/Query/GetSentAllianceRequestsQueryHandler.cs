...
{
    public class GetSentAllianceRequestsQueryHandler : IGetSentAllianceRequestsQuery
    {
        private readonly IPoliticalAllienceRepository _allianceRepository;

        public GetSentAllianceRequestsQueryHandler(IPoliticalAllienceRepository allianceRepository)
        {
        ...
        public async Task<ValidationResult<IEnumerable<AllianceDto>>> ExecuteAsync(int authenticatedPartyId)
        {
            ...
            try
            {
                var alliances = await _allianceRepository.GetSentRequestsAsync(authenticatedPartyId);
